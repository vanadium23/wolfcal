...
## Done summary
# Task fn-12-zlg.5: Write regression tests for BUG-5

## Summary

Created comprehensive regression tests for BUG-5 (UI update after sync) at `src/test/components/bugs-ui-state.test.tsx`. The tests document the bug behavior and will verify the fix when implemented in fn-12-zlg.11.

## Tests Created

12 tests covering:
1. **SyncStatusBar tests** - Verifying status display behavior
2. **Bug reproduction tests** - Confirming SyncScheduler lacks onSyncComplete callback
3. **Verification tests** - Documenting expected behavior after fix
4. **Code verification tests** - Confirming existing patterns (RefreshButton, Calendar, App)

## Key Findings

The tests confirm that:
- Manual sync works correctly (via RefreshButton -> onSyncComplete callback)
- Auto-sync does NOT trigger UI refresh (SyncScheduler has no callback mechanism)
- Calendar component already has correct useEffect watching refreshTrigger
- The fix requires connecting SyncScheduler to App component via callback
